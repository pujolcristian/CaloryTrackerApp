package com.cpujoldev.onboarding_presentation.height

import androidx.compose.runtime.getValue
import androidx.compose.runtime.mutableStateOf
import androidx.compose.runtime.setValue
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import com.cpujoldev.core.R
import com.cpujoldev.core.domain.preferences.Preferences
import com.cpujoldev.core.domain.use_case.FilterOutDigits
import com.cpujoldev.core.navigation.Route
import com.cpujoldev.core.util.UiEvent
import com.cpujoldev.core.util.UiText
import dagger.hilt.android.lifecycle.HiltViewModel
import kotlinx.coroutines.channels.Channel
import kotlinx.coroutines.flow.receiveAsFlow
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class HeightViewModel @Inject constructor(
    private val preferences: Preferences,
    private val filterOutDigits: FilterOutDigits
) : ViewModel() {

    var height by mutableStateOf("180")
        private set

    private val _uiEvent = Channel<UiEvent>()
    val uiEvent = _uiEvent.receiveAsFlow()

    fun onEvent(event: HeightEvent) {
        when (event) {
            is HeightEvent.OnHeightChange -> handleHeightChange(event.height)
            HeightEvent.OnNextClick -> handleNextClick()
        }
    }

    private fun handleHeightChange(height: String) {
        if (height.length <= 3) {
            this.height = filterOutDigits(height)
        }
    }

    private fun handleNextClick() {
        viewModelScope.launch {
            val heightNumber = height.toIntOrNull() ?: kotlin.run {
                showInvalidHeightError()
                return@launch
            }
            saveHeight(heightNumber)
            navigateToWeightScreen()
        }
    }

    private suspend fun showInvalidHeightError() {
        _uiEvent.send(UiEvent.ShowSnackBar(UiText.StringResource(R.string.error_height_cant_be_empty)))
    }

    private suspend fun navigateToWeightScreen() {
        _uiEvent.send(UiEvent.Navigate(Route.WEIGHT_SCREEN))
    }

    private fun saveHeight(height: Int) {
        preferences.saveHeight(height)
    }
}
